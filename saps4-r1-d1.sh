...
mount /dev/vg00/hana_shared /hana/shared
mount /dev/vg01/hana_data /hana/data
mount /dev/vg02/hana_log /hana/log
mount /dev/vg03/usr_sap /usr/sap
mount /dev/vg04/backup /backup
# 8. Print_fstab - Print mounts in the fstab:
echo "/dev/vg00/hana_shared /hana/shared xfs defaults 1 3" >> /etc/fstab
echo "/dev/vg01/hana_data /hana/data xfs defaults 1 3" >> /etc/fstab
echo "/dev/vg02/hana_log /hana/log xfs defaults 1 3" >> /etc/fstab
echo "/dev/vg03/usr_sap /usr/sap xfs defaults 1 3" >> /etc/fstab
echo "/dev/vg04/backup /backup xfs defaults 1 3" >> /etc/fstab
mount -a
# 9. Create_usrgrp - Create user/group:
groupadd sapsys -g 79
useradd -u 550 -g 79 batadm
chsh -s /bin/sh batadm
# 10. Set hostname - Short "hostname" that requires for sap installations.
hostnamectl set-hostname --static saps4-r1-d1
